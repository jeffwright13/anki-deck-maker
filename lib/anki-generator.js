...
class AnkiGenerator {
    constructor() {
        this.noteTypeId = Date.now(); // Unique note type ID
        this.deckId = Date.now() + 1; // Base deck ID
    ...
    generateGuid() {
        return Math.floor(Math.random() * 9223372036854775807) + 1;
    }
 ...
 font-size: 20px;
    ...
    insertNotesAndCards(db, cards, decks) {
        ...
        cards.forEach(card => {
            ...
            // Determine direction indicator based on card type
            const direction = card.type === 'recognition' ? 'ES → EN' : 'EN → ES';
            
            // Include direction as third field: Front, Back, Direction
            const fields = `${card.front}\x1f${card.back}\x1f${direction}`;
